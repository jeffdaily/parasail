        ...
        const int open, const int gap, const parasail_matrix_t *matrix)
{
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        vNmatch = _mm_srli_si128(vNmatch, 2);
        vNmatch = _mm_insert_epi16(vNmatch, 0, 7);
        vNsimilar = _mm_srli_si128(vNsimilar, 2);
        vNsimilar = _mm_insert_epi16(vNsimilar, 0, 7);
        vNlength = _mm_srli_si128(vNlength, 2);
        vNlength = _mm_insert_epi16(vNlength, 0, 7);
        vWscore = _mm_srli_si128(vWscore, 2);
        vWscore = _mm_insert_epi16(vWscore, -open - i*gap, 7);
        vWmatch = _mm_srli_si128(vWmatch, 2);
        vWmatch = _mm_insert_epi16(vWmatch, 0, 7);
        vWsimilar = _mm_srli_si128(vWsimilar, 2);
        vWsimilar = _mm_insert_epi16(vWsimilar, 0, 7);
        vWlength = _mm_srli_si128(vWlength, 2);
        vWlength = _mm_insert_epi16(vWlength, 0, 7);
    ...
    for (i=0; i<N; ++i) {
        ...
        if (value > score) {
            score = value;
            matches = (int16_t) _mm_extract_epi16(vMaxMatch, 7);
            similar = (int16_t) _mm_extract_epi16(vMaxSimilar, 7);
            length= (int16_t) _mm_extract_epi16(vMaxLength, 7);
        }
        vMaxScore = _mm_slli_si128(vMaxScore, 2);
        vMaxMatch = _mm_slli_si128(vMaxMatch, 2);
        vMaxSimilar = _mm_slli_si128(vMaxSimilar, 2);

        ...
        const int open, const int gap, const parasail_matrix_t *matrix)
{
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        vNscore = _mm_insert_epi8(vNscore, tbl_pr[-1], 15);
        vNmatch = _mm_srli_si128(vNmatch, 1);
        vNmatch = _mm_insert_epi8(vNmatch, 0, 15);
        vNsimilar = _mm_srli_si128(vNsimilar, 1);
        vNsimilar = _mm_insert_epi8(vNsimilar, 0, 15);
        vNlength = _mm_srli_si128(vNlength, 1);
        vNlength = _mm_insert_epi8(vNlength, 0, 15);
        vWscore = _mm_srli_si128(vWscore, 1);
        vWscore = _mm_insert_epi8(vWscore, -open - i*gap, 15);
        vWmatch = _mm_srli_si128(vWmatch, 1);
        vWmatch = _mm_insert_epi8(vWmatch, 0, 15);
        vWsimilar = _mm_srli_si128(vWsimilar, 1);
        vWsimilar = _mm_insert_epi8(vWsimilar, 0, 15);
        vWlength = _mm_srli_si128(vWlength, 1);
        vWlength = _mm_insert_epi8(vWlength, 0, 15);

...
#ifdef PARASAIL_TABLE
#define FNAME nw_stats_table_striped_avx2_256_32
#else
#define FNAME nw_stats_striped_avx2_256_32
#endif
        ...
        const int open, const int gap, const int matrix[24][24])
{
    ...
    initialF = _mm256_add_epi32(initialF, vGapE);
    ...
    for (j=0; j<s2Len; ++j) {
         ...
         * in the Lazy_F loop.  */
        initialF = _mm256_sub_epi32(initialF, vGapE);
        vF = initialF;
        vFM = vZero;
        vFL = vZero;
        ...
        for (k=0; k<segWidth; ++k) {
            ...
            vF = shift(vF);
            vF = _mm256_insert_epi32(vF, boundary[j+1]-open, 0);
            vFM = shift(vFM);
            vFL = shift(vFL);
            for (i=0; i<segLen; ++i) {
                __m256i case1not;

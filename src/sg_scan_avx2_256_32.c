...
static inline __m256i shift(__m256i a, __m256i idx) {
    ...
    return _mm256_insert_epi32(result, 0, 0);
#else
    __m256i_32_t tmp;
    tmp.m = a;
    tmp.v[7] = tmp.v[6];
        ...
        const int open, const int gap, const int matrix[24][24])
{
    ...
    for (j=0; j<s2Len; ++j) {
        ...
        /* extract last value from column */
        {
            ...
            value = (int32_t) _mm256_extract_epi32(vH, 7);
            if (value > score) {
                score = value;
            }
        }

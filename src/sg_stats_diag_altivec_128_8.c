...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sg_stats_table_diag_altivec_128_8
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sg_stats_rowcol_diag_altivec_128_8
#else
#define FNAME parasail_sg_stats_diag_altivec_128_8
#endif
#endif
        ...
        const int open, const int gap, const parasail_matrix_t *matrix)
{
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        for (j=0; j<s2Len+PAD; ++j) {
            ...
            {
                vec128i vJeqLimit1 = _mm_cmpeq_epi8(vJ, vJLimit1);
                vec128i vJgtNegOne = _mm_cmpgt_epi8(vJ, vNegOne);
                vec128i vJltLimit = _mm_cmplt_epi8(vJ, vJLimit);

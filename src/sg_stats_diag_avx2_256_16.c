...
{
    __m256i ret = _mm256_srli_si256_rpl(v, 2);
...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sg_stats_table_diag_avx2_256_16
#else
#define FNAME parasail_sg_stats_diag_avx2_256_16
#endif
        ...
        const int open, const int gap, const parasail_matrix_t *matrix)
{
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        for (j=0; j<s2Len+PAD; ++j) {
            ...
            sim_pr[j-15] = (int16_t)_mm256_extract_epi16(vWsimilar,0);
    ...
    for (i=0; i<N; ++i) {
        ...
        if (value > score) {
            ...
            similar = (int16_t) _mm256_extract_epi16(vMaxSimilar, 15);

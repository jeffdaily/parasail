...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sg_flags_stats_table_diag_sse2_128_32
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sg_flags_stats_rowcol_diag_sse2_128_32
#else
#define FNAME parasail_sg_flags_stats_diag_sse2_128_32
#endif
#endif
        ...
        int s1_beg, int s1_end, int s2_beg, int s2_end)
{
    ...
    __m128i vMaxMRow = vNegInf;
    __m128i vMaxSRow = vNegInf;
    __m128i vMaxLRow = vNegInf;
    __m128i vMaxHCol = vNegInf;
    __m128i vMaxMCol = vNegInf;
    __m128i vMaxSCol = vNegInf;
    __m128i vMaxLCol = vNegInf;
    __m128i vLastValH = vNegInf;
    __m128i vLastValM = vNegInf;
    __m128i vLastValS = vNegInf;
    __m128i vLastValL = vNegInf;
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        for (j=0; j<s2Len+PAD; ++j) {
            ...
            vNM = _mm_insert_epi32_rpl(vNM, HM_pr[j], 3);
            vNS = _mm_srli_si128(vWS, 4);
            vNS = _mm_insert_epi32_rpl(vNS, HS_pr[j], 3);
            vNL = _mm_srli_si128(vWL, 4);
            vNL = _mm_insert_epi32_rpl(vNL, HL_pr[j], 3);
            vF = _mm_srli_si128(vF, 4);
            vF = _mm_insert_epi32_rpl(vF, F_pr[j], 3);
            vFM = _mm_srli_si128(vFM, 4);
            vFM = _mm_insert_epi32_rpl(vFM, FM_pr[j], 3);
            vFS = _mm_srli_si128(vFS, 4);
            vFS = _mm_insert_epi32_rpl(vFS, FS_pr[j], 3);
            vFL = _mm_srli_si128(vFL, 4);
            vFL = _mm_insert_epi32_rpl(vFL, FL_pr[j], 3);
            ...
            vSaturationCheckMax = _mm_max_epi32_rpl(vSaturationCheckMax, vWM);
            vSaturationCheckMax = _mm_max_epi32_rpl(vSaturationCheckMax, vWS);
            vSaturationCheckMax = _mm_max_epi32_rpl(vSaturationCheckMax, vWL);
            vSaturationCheckMax = _mm_max_epi32_rpl(vSaturationCheckMax, vWL);
            vSaturationCheckMax = _mm_max_epi32_rpl(vSaturationCheckMax, vJ);
#ifdef PARASAIL_TABLE
            ...
            HM_pr[j-3] = (int32_t)_mm_extract_epi32_rpl(vWM,0);
            HS_pr[j-3] = (int32_t)_mm_extract_epi32_rpl(vWS,0);
            HL_pr[j-3] = (int32_t)_mm_extract_epi32_rpl(vWL,0);
            F_pr[j-3] = (int32_t)_mm_extract_epi32_rpl(vF,0);
            FM_pr[j-3] = (int32_t)_mm_extract_epi32_rpl(vFM,0);
            FS_pr[j-3] = (int32_t)_mm_extract_epi32_rpl(vFS,0);
            FL_pr[j-3] = (int32_t)_mm_extract_epi32_rpl(vFL,0);
             ...
             * boundary, extract the last value of the column or row */
            {
                ...
                vMaxMRow = _mm_blendv_epi8_rpl(vMaxMRow, vWM, cond_all_row);
                vMaxSRow = _mm_blendv_epi8_rpl(vMaxSRow, vWS, cond_all_row);
                vMaxLRow = _mm_blendv_epi8_rpl(vMaxLRow, vWL, cond_all_row);
                vMaxHCol = _mm_blendv_epi8_rpl(vMaxHCol, vWH, cond_all_col);
                vMaxMCol = _mm_blendv_epi8_rpl(vMaxMCol, vWM, cond_all_col);
                vMaxSCol = _mm_blendv_epi8_rpl(vMaxSCol, vWS, cond_all_col);
                vMaxLCol = _mm_blendv_epi8_rpl(vMaxLCol, vWL, cond_all_col);
                vLastValH = _mm_blendv_epi8_rpl(vLastValH, vWH, cond_last_val);
                vLastValM = _mm_blendv_epi8_rpl(vLastValM, vWM, cond_last_val);
                vLastValS = _mm_blendv_epi8_rpl(vLastValS, vWS, cond_last_val);
                vLastValL = _mm_blendv_epi8_rpl(vLastValL, vWL, cond_last_val);
        ...
        vSaturationCheckMax = _mm_max_epi32_rpl(vSaturationCheckMax, vI);
    }

...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sg_flags_stats_table_striped_neon_128_32
#define PNAME parasail_sg_flags_stats_table_striped_profile_neon_128_32
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sg_flags_stats_rowcol_striped_neon_128_32
#define PNAME parasail_sg_flags_stats_rowcol_striped_profile_neon_128_32
#else
#define FNAME parasail_sg_flags_stats_striped_neon_128_32
#define PNAME parasail_sg_flags_stats_striped_profile_neon_128_32
#define INAME parasail_sg_flags_stats_striped_profile_neon_128_32_internal
#endif
#endif
        ...
        int s1_beg, int s1_end, int s2_beg, int s2_end)
{
    ...
    const simde__m128i vOne = simde_mm_set1_epi32(1);
...

    {
        ...
        for (k=0; k<position; ++k) {
            ...
            vMaxHM = simde_mm_slli_si128 (vMaxHM, 4);
            vMaxHS = simde_mm_slli_si128 (vMaxHS, 4);
            vMaxHL = simde_mm_slli_si128 (vMaxHL, 4);
        }
        score = (int32_t) simde_mm_extract_epi32 (vMaxH, 3);
        matches = (int32_t)simde_mm_extract_epi32(vMaxHM, 3);
        similar = (int32_t)simde_mm_extract_epi32(vMaxHS, 3);
        length = (int32_t)simde_mm_extract_epi32(vMaxHL, 3);
    }
        ...
        int s1_beg, int s1_end, int s2_beg, int s2_end)
{
    ...
    parasail_result_t *result = parasail_sg_flags_striped_profile_neon_128_32(profile, s2, s2Len, open, gap, s1_beg, s1_end, s2_beg, s2_end);

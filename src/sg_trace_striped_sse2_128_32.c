...
#define FNAME parasail_sg_flags_trace_striped_sse2_128_32
#define PNAME parasail_sg_flags_trace_striped_profile_sse2_128_32
        ...
        int s1_beg, int s1_end, int s2_beg, int s2_end)
{
    ...
    __m128i vTMask = _mm_set1_epi32(PARASAIL_ZERO_MASK);
    __m128i vFTMask = _mm_set1_epi32(PARASAIL_F_MASK);
    ...
    for (j=0; j<s2Len; ++j) {
        ...
        for (k=0; k<segWidth; ++k) {
            ...
            vEF_opn = _mm_insert_epi32_rpl(vEF_opn, tmp2, 0);
            ...
            vFa = _mm_insert_epi32_rpl(vFa, tmp2, 0);
            for (i=0; i<segLen; ++i) {

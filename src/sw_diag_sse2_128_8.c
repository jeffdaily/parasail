...
#ifdef PARASAIL_TABLE
#define FNAME sw_table_diag_sse2_128_8
#else
#define FNAME sw_diag_sse2_128_8
#endif
        ...
        const int open, const int gap, const int matrix[24][24])
{
...
#endif
            tbl_pr[j-15] = (int16_t)_mm_extract_epi8_rpl(vWscore,0);
            del_pr[j-15] = (int16_t)_mm_extract_epi8_rpl(vDel,0);
    ...
    for (i=0; i<N; ++i) {
        int16_t value;
        value = (int16_t) _mm_extract_epi8_rpl(vMax, 15);
        if (value > score) {
            score = value;
        }

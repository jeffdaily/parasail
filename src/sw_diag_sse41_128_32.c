...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sw_table_diag_sse41_128_32
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sw_rowcol_diag_sse41_128_32
#else
#define FNAME parasail_sw_diag_sse41_128_32
#endif
#endif
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        for (j=0; j<s2Len+PAD; ++j) {
             ...
             * max values within the i,j bounds */
            {
                ...
                vCompare = _mm_cmpgt_epi32(vMax, vMaxUnit);
                if (_mm_movemask_epi8(vCompare)) {
                    score = _mm_hmax_epi32_rpl(vMax);
                    vMaxUnit = _mm_set1_epi32(score);
                    vEndH = vMax;
                    vEndI = vI;
                    vEndJ = vJ;
                }

...
#ifdef PARASAIL_TABLE
#define FNAME sw_table_diag_sse41_128_64
#else
#define FNAME sw_diag_sse41_128_64
#endif
        ...
        const int open, const int gap, const int matrix[24][24])
{
...
#endif
            tbl_pr[j-1] = (int16_t)_mm_extract_epi64(vWscore,0);
            del_pr[j-1] = (int16_t)_mm_extract_epi64(vDel,0);
    ...
    for (i=0; i<N; ++i) {
        int16_t value;
        value = (int16_t) _mm_extract_epi64(vMax, 1);
        if (value > score) {
            score = value;
        }

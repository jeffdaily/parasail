...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sw_stats_table_diag_avx2_256_32
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sw_stats_rowcol_diag_avx2_256_32
#else
#define FNAME parasail_sw_stats_diag_avx2_256_32
#endif
#endif
        ...
        const int open, const int gap, const parasail_matrix_t *matrix)
{
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        for (j=0; j<s2Len+PAD; ++j) {
            ...
            {
                __m256i cond_zero = _mm256_cmpeq_epi32(vWscore, vZero);

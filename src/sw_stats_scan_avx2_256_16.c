...
static inline void arr_store_si256(
        ...
        int32_t dlen)
{
    array[( 0*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  0);
    array[( 1*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  1);
    array[( 2*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  2);
    array[( 3*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  3);
    array[( 4*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  4);
    array[( 5*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  5);
    array[( 6*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  6);
    array[( 7*seglen+t)*dlen + d] = (int32_t)_mm256_extract_epi16(vH,  7);
...
#ifdef PARASAIL_TABLE
#define FNAME sw_stats_table_scan_avx2_256_16
#else
#define FNAME sw_stats_scan_avx2_256_16
#endif
        ...
        const int open, const int gap, const int matrix[24][24])
{
    ...
    for (j=0; j<s2Len; ++j) {
        ...
        for (i=0; i<segLen; ++i) {
            ...
            vHt = _mm256_max_epi16(vHt, vZero);
            /* statistics */
            ...
            cond_max = _mm256_cmpeq_epi16(vHt, vZero);
    ...
    for (j=0; j<segWidth; ++j) {
        ...
        if (value > score) {
            matches = (int16_t) _mm256_extract_epi16(vMaxM, 15);
            similar = (int16_t) _mm256_extract_epi16(vMaxS, 15);
            length = (int16_t) _mm256_extract_epi16(vMaxL, 15);
            score = value;
        }
        vMaxH = shift(vMaxH);
        vMaxM = shift(vMaxM);

        ...
        const int open, const int gap)
{
    ...
    __m256i vAll = _mm256_cmpeq_epi8(vZero,vZero);
        ...
        for (k=0; k<segWidth; ++k) {
            ...
            for (i=0; i<segLen; ++i) {
                ...
                if (! _mm256_movemask_epi8(
                            _mm256_or_si256(
                                _mm256_cmpgt_epi8(vF_ext, vF_opn),
                                _mm256_and_si256(
                                    _mm256_cmpeq_epi8(vF_ext, vF_opn),
                                    _mm256_cmpgt_epi8(vF_ext, vBias)))))
                    goto end;
                /*vF = _mm256_max_epi8(vF_opn, vF_ext);*/
                vF = vF_ext;
                cond = _mm256_cmpgt_epi8(vF_opn, vF_ext);
        ...
        const int open, const int gap)
{
    ...
    if (!result->saturated) {
        ...
        profile_new = parasail_profile_create_avx_256_8(
                s1_new, s1Len_new, matrix);
        profile_new->stop = result->score;
        result_new = parasail_sw_striped_profile_avx2_256_8(

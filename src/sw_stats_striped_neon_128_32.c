...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sw_stats_table_striped_neon_128_32
#define PNAME parasail_sw_stats_table_striped_profile_neon_128_32
#define INAME PNAME
#define STATIC
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sw_stats_rowcol_striped_neon_128_32
#define PNAME parasail_sw_stats_rowcol_striped_profile_neon_128_32
#define INAME PNAME
#define STATIC
#else
#define FNAME parasail_sw_stats_striped_neon_128_32
#ifdef FASTSTATS
#define PNAME parasail_sw_stats_striped_profile_neon_128_32_internal
#define INAME parasail_sw_stats_striped_profile_neon_128_32
#define STATIC static
#else
#define PNAME parasail_sw_stats_striped_profile_neon_128_32
#define INAME PNAME
#define STATIC
#endif
#endif
        ...
        const int open, const int gap)
{
    ...
    simde__m128i* const restrict vProfileM = (simde__m128i*)profile->profile32.matches;
    simde__m128i* const restrict vProfileS = (simde__m128i*)profile->profile32.similar;
    ...
    simde__m128i vPosLimit = simde_mm_set1_epi32(INT32_MAX);
    ...
    for (j=0; j<s2Len; ++j) {
        ...
        for (i=0; i<segLen; ++i) {
            ...
            vHM = simde_mm_blendv_epi8(
                    ...
                    simde_mm_add_epi32(vHM, simde_mm_load_si128(vPM + i)), case1);
            ...
            vHS = simde_mm_blendv_epi8(
                    ...
                    simde_mm_add_epi32(vHS, simde_mm_load_si128(vPS + i)), case1);
            ...
            vHL = simde_mm_blendv_epi8(
                    ...
                    simde_mm_add_epi32(vHL, vOne), case1);
        ...
        for (k=0; k<segWidth; ++k) {
            ...
            for (i=0; i<segLen; ++i) {
                ...
                vHp = simde_mm_max_epi32(vHp, vZero);
                ...
                if (! simde_mm_movemask_epi8(
                            simde_mm_or_si128(
                                ...
                                simde_mm_and_si128(
                                    ...
                                    simde_mm_cmpgt_epi32(vF_ext, vZero)))))
                    goto end;
                /*vF = simde_mm_max_epi32(vEF_opn, vF_ext);*/
                vF = vF_ext;
    ...
    if (score == INT32_MAX
            || simde_mm_movemask_epi8(simde_mm_cmpeq_epi32(vSaturationCheckMax,vPosLimit))) {
        result->flag |= PARASAIL_FLAG_SATURATED;
    }
        ...
        const int open, const int gap)
{
    ...
    if (!parasail_result_is_saturated(result)) {
        ...
        profile_new = parasail_profile_create_neon_128_32(
                s1_new, s1Len_new, matrix);
        profile_new->stop = result->score;
        result_new = parasail_sw_striped_profile_neon_128_32(
        ...
        profile_final = parasail_profile_create_stats_neon_128_32(

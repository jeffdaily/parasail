...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sw_stats_table_striped_neon_128_8
#define PNAME parasail_sw_stats_table_striped_profile_neon_128_8
#define INAME PNAME
#define STATIC
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sw_stats_rowcol_striped_neon_128_8
#define PNAME parasail_sw_stats_rowcol_striped_profile_neon_128_8
#define INAME PNAME
#define STATIC
#else
#define FNAME parasail_sw_stats_striped_neon_128_8
#ifdef FASTSTATS
#define PNAME parasail_sw_stats_striped_profile_neon_128_8_internal
#define INAME parasail_sw_stats_striped_profile_neon_128_8
#define STATIC static
#else
#define PNAME parasail_sw_stats_striped_profile_neon_128_8
#define INAME PNAME
#define STATIC
#endif
#endif
        ...
        const int open, const int gap)
{
    ...
    simde__m128i* const restrict vProfileM = (simde__m128i*)profile->profile8.matches;
    simde__m128i* const restrict vProfileS = (simde__m128i*)profile->profile8.similar;
    ...
    simde__m128i vBias1 = simde_mm_adds_epi8(vBias,vOne);
    ...
    simde__m128i insert_mask = simde_mm_cmpgt_epi8(
            ...
            vZero);
    simde__m128i vSaturationCheckMax = vBias;
    ...
    for (j=0; j<s2Len; ++j) {
        ...
        for (i=0; i<segLen; ++i) {
            ...
            vHM = simde_mm_blendv_epi8(
                    ...
                    simde_mm_adds_epi8(vHM, simde_mm_load_si128(vPM + i)), case1);
            ...
            vHS = simde_mm_blendv_epi8(
                    ...
                    simde_mm_adds_epi8(vHS, simde_mm_load_si128(vPS + i)), case1);
            ...
            vHL = simde_mm_blendv_epi8(
                    ...
                    simde_mm_adds_epi8(vHL, vOne), case1);
                ...
                if (! simde_mm_movemask_epi8(
                            simde_mm_or_si128(
                                ...
                                simde_mm_and_si128(
                                    ...
                                    simde_mm_cmpgt_epi8(vF_ext, vBias)))))
                    goto end;
                /*vF = simde_mm_max_epi8(vEF_opn, vF_ext);*/
                vF = vF_ext;
        ...
        /* extract last value from the column */
        {
            ...
            result->stats->rowcols->score_row[j] = (int8_t) simde_mm_extract_epi8 (vH, 15) - bias;
            result->stats->rowcols->matches_row[j] = (int8_t) simde_mm_extract_epi8 (vHM, 15) - bias;
            result->stats->rowcols->similar_row[j] = (int8_t) simde_mm_extract_epi8 (vHS, 15) - bias;
            result->stats->rowcols->length_row[j] = (int8_t) simde_mm_extract_epi8 (vHL, 15) - bias;
        }
#endif
    ...
    if (score == INT8_MAX
            || simde_mm_movemask_epi8(simde_mm_cmpeq_epi8(vSaturationCheckMax,vPosLimit))) {
        result->flag |= PARASAIL_FLAG_SATURATED;
    }
        ...
        const int open, const int gap)
{
    ...
    if (!parasail_result_is_saturated(result)) {
        ...
        profile_new = parasail_profile_create_neon_128_8(
                s1_new, s1Len_new, matrix);
        profile_new->stop = result->score;
        result_new = parasail_sw_striped_profile_neon_128_8(
        ...
        profile_final = parasail_profile_create_stats_neon_128_8(

...
static inline __m128i _mm_insert_epi8(__m128i a, int i, int imm) {
    __m128i_8_t tmp;
    tmp.m = a;
    tmp.v[imm] = i;
    return tmp.m;
        ...
        const int open, const int gap, const int matrix[24][24])
{
    ...
    __m128i* const restrict vProfileM = parasail_memalign_m128i(16, n * segLen);
    ...
    __m128i* restrict pvHSStore       = parasail_memalign_m128i(16, segLen);
    __m128i* restrict pvHSLoad        = parasail_memalign_m128i(16, segLen);
    ...
    __m128i* restrict pvES            = parasail_memalign_m128i(16, segLen);
    ...
    parasail_memset_m128i(pvHSStore, vNegInf, segLen);
    ...
    for (j=0; j<s2Len; ++j) {
        ...
        for (i=0; i<segLen; ++i) {
            ...
            vHM = _mm_or_si128(vHM, _mm_and_si128(cond_zero, vNegInf));
            _mm_store_si128(pvHMStore + i, vHM);

            /* calculate vS */
            vES = _mm_load_si128(pvES + i);
            vHS = _mm_andnot_si128(case1not,
                    _mm_adds_epi8(vHS, _mm_load_si128(vPS + i)));
            vHS = _mm_or_si128(vHS, _mm_and_si128(case2, vFS));
            vHS = _mm_or_si128(vHS, _mm_and_si128(case3, vES));
            vHS = _mm_andnot_si128(cond_zero, vHS);
            vHS = _mm_or_si128(vHS, _mm_and_si128(cond_zero, vNegInf));
            ...
            vHL = _mm_or_si128(vHL, _mm_and_si128(cond_zero, vNegInf));
            ...
            /* update max vector seen so far */
            {
                ...
                vMaxS = _mm_or_si128(vMaxS, _mm_and_si128(cond_max, vHS));
        ...
        for (k=0; k<segWidth; ++k) {
            ...
            for (i=0; i<segLen; ++i) {
                ...
                vHS = _mm_andnot_si128(case2, vHS);
                vHS = _mm_or_si128(vHS, _mm_and_si128(case2, vFS));
                vHS = _mm_andnot_si128(cond_zero, vHS);
                vHS = _mm_or_si128(vHS, _mm_and_si128(cond_zero, vNegInf));
                _mm_store_si128(pvHSStore + i, vHS);
    ...
    for (j=0; j<segWidth; ++j) {
        ...
        if (value > score) {
            ...
            similar = (int8_t) _mm_extract_epi8(vMaxS, 15) - (int)bias;

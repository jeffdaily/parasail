...
#ifdef PARASAIL_TABLE
#define FNAME parasail_sw_stats_table_striped_sse41_128_8
#define PNAME parasail_sw_stats_table_striped_profile_sse41_128_8
#define INAME PNAME
#define STATIC
#else
#ifdef PARASAIL_ROWCOL
#define FNAME parasail_sw_stats_rowcol_striped_sse41_128_8
#define PNAME parasail_sw_stats_rowcol_striped_profile_sse41_128_8
#define INAME PNAME
#define STATIC
#else
#define FNAME parasail_sw_stats_striped_sse41_128_8
#ifdef FASTSTATS
#define PNAME parasail_sw_stats_striped_profile_sse41_128_8_internal
#define INAME parasail_sw_stats_striped_profile_sse41_128_8
#define STATIC static
#else
#define PNAME parasail_sw_stats_striped_profile_sse41_128_8
#define INAME PNAME
#define STATIC
#endif
#endif
        ...
        const int open, const int gap)
{
    ...
    __m128i vAll = _mm_cmpeq_epi8(vZero,vZero);
    ...
    for (j=0; j<s2Len; ++j) {
        ...
        for (k=0; k<segWidth; ++k) {
            ...
            for (i=0; i<segLen; ++i) {
                ...
                if (! _mm_movemask_epi8(
                            _mm_or_si128(
                                _mm_cmpgt_epi8(vF_ext, vF_opn),
                                _mm_and_si128(
                                    _mm_cmpeq_epi8(vF_ext, vF_opn),
                                    _mm_cmpgt_epi8(vF_ext, vBias)))))
                    goto end;
                /*vF = _mm_max_epi8(vF_opn, vF_ext);*/
                vF = vF_ext;
                cond = _mm_cmpgt_epi8(vF_opn, vF_ext);
        ...
        /* extract last value from the column */
        {
            ...
            result->matches_row[j] = (int8_t) _mm_extract_epi8 (vHM, 15) - bias;
            result->similar_row[j] = (int8_t) _mm_extract_epi8 (vHS, 15) - bias;
            result->length_row[j] = (int8_t) _mm_extract_epi8 (vHL, 15) - bias;
        }
#endif
    }

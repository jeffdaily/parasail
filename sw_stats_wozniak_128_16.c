...
#ifdef ALIGN_EXTRA
#define FNAME sw_stats_wozniak_128_16_debug
#else
#define FNAME sw_stats_wozniak_128_16
#endif
int FNAME(
        ...
        )
{
    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        for (j=0; j<s2Len+PAD2; ++j) {
            ...
            mch_pr[j-7] = _mm_extract_epi16(vWmatch,0);
            len_pr[j-7] = _mm_extract_epi16(vWlength,0);

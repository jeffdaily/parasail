    ...
    for (i=0; i<s1Len; i+=N) {
        ...
        for (j=0; j<s2Len+PAD; ++j) {
             ...
             * assign the appropriate boundary conditions */
            {
                ...
                vWH = %(VKANDNOT)s(cond, vWH);
                vWM = %(VKANDNOT)s(cond, vWM);
                vWS = %(VKANDNOT)s(cond, vWS);
                vWL = %(VKANDNOT)s(cond, vWL);
                vE = %(VBLEND)s(vE, vNegInf, cond);
                vEM = %(VKANDNOT)s(cond, vEM);
                vES = %(VKANDNOT)s(cond, vES);
                vEL = %(VKANDNOT)s(cond, vEL);
            }
            ...
            arr_store_si%(BITS)s(result->similar_table, vWS, i, s1Len, j, s2Len);
            arr_store_si%(BITS)s(result->length_table, vWL, i, s1Len, j, s2Len);
#endif
#ifdef PARASAIL_ROWCOL
             ...
             * boundary, extract the last value of the column or row */
            {
                ...
                %(VTYPE)s cond_j = %(VKAND)s(vIltLimit, vJeqLimit1);
                %(VTYPE)s cond_i = %(VKAND)s(vIeqLimit1,
                        %(VKAND)s(vJgtNegOne, vJltLimit));
                %(VTYPE)s cond_valid_IJ = %(VKOR)s(cond_i, cond_j);
                %(VTYPE)s cond_eq = %(VCMPEQ)s(vWH, vMaxH);
                %(VTYPE)s cond_max = %(VCMPGT)s(vWH, vMaxH);
                %(VTYPE)s cond_all = %(VKAND)s(cond_max, cond_valid_IJ);
                ...
                cond_all = %(VKAND)s(cond_Jlt, cond_eq);
                cond_all = %(VKAND)s(cond_all, cond_valid_IJ);
                vMaxM = %(VBLEND)s(vMaxM, vWM, cond_all);
                vMaxS = %(VBLEND)s(vMaxS, vWS, cond_all);
                vMaxL = %(VBLEND)s(vMaxL, vWL, cond_all);
                vEndI = %(VBLEND)s(vEndI, vI, cond_all);

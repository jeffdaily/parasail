        ...
        const int open, const int gap)
{
    ...
    vGapE = %(VSET1)s(gap);
    vZero = %(VSET1)s(0);
    bias = INT%(WIDTH)s_MIN;
    score = bias;
    vBias = %(VSET1)s(bias);
    vMaxH = vBias;
    vMaxHUnit = vBias;
    maxp = INT%(WIDTH)s_MAX - (%(INT)s)(matrix->max+1);
    insert_mask = %(VCMPGT)s(
            %(VSET)s(%(STRIPED_INSERT_MASK)s),
            vZero);
    /*stop = profile->stop == INT32_MAX ?  INT%(WIDTH)s_MAX : (%(INT)s)profile->stop-bias;*/
    ...
    for (j=0; j<s2Len; ++j) {
...
#ifdef PARASAIL_TABLE
            arr_store_si%(BITS)s(result->tables->score_table, vH, i, segLen, j, s2Len, bias);
#endif
        ...
        /* extract last value from the column */
        {
            ...
            result->rowcols->score_row[j] = (%(INT)s) %(VEXTRACT)s (vH, %(LAST_POS)s) - bias;
        }
#endif
